\documentclass[14pt,aspectratio=169,xcolor=dvipsnames]{beamer}
\usetheme{SimplePlus}
\usepackage{booktabs}
\usepackage{minted}

\title[short title]{Clase 01 Introducción a la Ingeniería Matemática}
\subtitle{}
\author[NA Barnafi] {Nicolás Alejandro Barnafi Wittwer}
\institute[UC|CMM] 
{
    Pontificia Universidad Católica de Chile \\
    Centro de Modelamiento Matemático
}

\titlegraphic{
    \vspace{-1.8cm}
    \begin{flushright}
      \includegraphics[height=2.5cm]{../images/puc.png} 
    \end{flushright}
}

\date{10/03/2024}
%\setbeamercovered{transparent}

\begin{document}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
    \maketitle
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}\frametitle{El curso}
    \begin{itemize}
        \item Clases lunes y miércoles
        \item "Ayudantía" los miércoles en la tarde
        \item Muchas tareas
        \item Lógica del curso: ensuciarse las manos "sin pensar en notas"
    \end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Qué es la ingeniería matemática}
    \pause
    \begin{block}{}
        Solución de problemas de ingeniería a través de matemática
    \end{block}
    \pause o
    \begin{block}{}
        Disciplina que estudia herramientas matemáticas para la solución de problemas de ingeniería
    \end{block}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{El IMC}
    \begin{itemize}
        \item Edificio de Carreras Interdisciplinarias
        \item 26 académicos + 10 administrativos + Uds
        \item Áreas:
            \begin{itemize}
                \item \alertGreen{Cómputo científico y análisis numérico}
                \item Algoritmos, combinatoria y optimización
                \item Aprendizaje estadístico y ciencia de datos
            \end{itemize}
    \end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Mi investigación}
    \begin{itemize}
        \item Algoritmos "escalables"
        \item Modelos físicos
        \item Principalmente en biomedicina
        \item Áreas relacionadas: EDP, AF, ALN, Opt
    \end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{frame}{Ejemplos}
%    \begin{enumerate}
%        \item Mejorando inv. prop
%        \item Vendedor viajero
%        \item Elasticidad
%    \end{enumerate}
%\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
    \maketitle
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{document}
